                ...
                if (result.equalsIgnoreCase("error")) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                        ...
                        if (fileDetail.fileType != 2) {
                            f = new File("file://" + Environment.getExternalStoragePublicDirectory(Environment.getExternalStorageDirectory() + Constants.IMAGE_DIRECTORY_LOCATION));
                        } else {
                        ...
                        if (fileDetail.fileType != 2) {
                            context.sendBroadcast(new Intent(Intent.ACTION_MEDIA_MOUNTED, Uri.parse("file://" + Environment.getExternalStorageDirectory() + Constants.IMAGE_DIRECTORY_LOCATION)));
                        } else {
                            context.sendBroadcast(new Intent(Intent.ACTION_MEDIA_MOUNTED, Uri.parse("file://" + Environment.getExternalStorageDirectory() + Constants.VIDEO_DIRECTORY_LOCATION)));
                        }
                    }
